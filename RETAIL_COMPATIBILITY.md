...
| Banking: GetNumBankSlots, classic main bank/reagent bank/bank bags | Current Retail uses returned character and Account/Warband bank tab IDs. `BankDocumentation.lua` and `Blizzard_UIPanels_Game/Mainline/BankFrame.lua`: C_Bank.CanViewBank, FetchNumPurchasedBankTabs, FetchPurchasedBankTabIDs and C_Container support both domains. Character tabs stay in `[BANK]`; account tabs are a distinct `[ACCOUNT BANK]` scope and root SavedVariables observation. Do not read negative Classic container IDs or claim an unviewable account bank is empty. |
| Bank freshness/events | BANKFRAME_OPENED/CLOSED and PLAYERBANKSLOTS_CHANGED remain; BANK_TABS_CHANGED(bankType), BANK_TAB_SETTINGS_UPDATED(bankType), and PLAYER_ACCOUNT_BANK_TAB_SLOTS_CHANGED are modern events. Classic PLAYERBANKBAGSLOTS_CHANGED may be absent (existing optional registration tolerates this). Viewability gates each domain even during an open frame. Failed/settling reads preserve the previous scope-specific observation and honest stale state. Guild Bank is a distinct legacy API/UI: current Retail live validation established `GuildBankFrame` OnShow/OnHide as its production lifecycle; documented GUILDBANKFRAME_OPENED/CLOSED are supplementary only. One viewable tab is queried at a time and `GUILDBANKBAGSLOTS_CHANGED` is attributed only to that sole outstanding query. |
...
Retail bank coverage explicitly identifies CHARACTER and ACCOUNT/Warband tabs as
separate ownership scopes; legacy reagent storage is not applicable. No combined
character/account totals are produced.

Warband bank is observed only when `C_Bank.CanViewBank(Enum.BankType.Account)` is
true and every returned purchased tab has settled container data. Guild Bank is not
represented as empty when inaccessible or unvisited; it remains UNKNOWN.
Guild Bank is Retail-only external shared storage, keyed by `C_Club.GetGuildClubId`
instead of account or character identity. `GetGuildBankTabInfo(...).canView` defines
the currently permitted tab set. A requested tab is read only after its serialized
query response; pre-response all-nil slots are UNKNOWN, while post-response all-nil
slots are OBSERVED empty. No concurrent Guild Bank requests are issued because the
response event has no tab payload. Inaccessible tabs are represented explicitly and
are not included in the complete observation of permitted tabs.
The current Retail implementation was live-validated with populated and genuinely
empty tabs. A closed/unavailable Guild Bank renders the prior complete observation
as LAST_SEEN; aborted or partial refreshes never replace it. Empty-slot presentation
defaults (`locked=false`, `isFiltered=false`) are not item evidence.
