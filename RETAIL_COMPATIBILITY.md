...
| Bank freshness/events | BANKFRAME_OPENED/CLOSED and PLAYERBANKSLOTS_CHANGED remain; BANK_TABS_CHANGED(bankType), BANK_TAB_SETTINGS_UPDATED(bankType), and PLAYER_ACCOUNT_BANK_TAB_SLOTS_CHANGED are modern events. Classic PLAYERBANKBAGSLOTS_CHANGED may be absent (existing optional registration tolerates this). Viewability gates each domain even during an open frame. Failed/settling reads preserve the previous scope-specific observation and honest stale state. Guild Bank uses a separate legacy API/UI and remains unsupported pending live passive-loading validation. |
