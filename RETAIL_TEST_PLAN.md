...
| Guild Bank | Open a Guild Bank with both viewable and non-viewable tabs if possible. Record passive `GetGuildBankTabInfo`/item loading behavior without issuing `QueryGuildBankTab`. Current package deliberately leaves it UNKNOWN pending proof that all readable tabs can be passively observed. | Pending live API validation |
